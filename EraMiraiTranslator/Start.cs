...
public static class Start
{
    ...
    public static void Main()
    {
        ...
        while (true)
        {
            ...
            const string menuString = """
                                      ...
                                      [ 9] - 访问项目主页
                                      """;
            var command = Tools.ReadLine(menuString);
            switch (command)
            {
                case "0":
                    Translator();
  ...
  public static Encoding GetEncoding(string filename)
    ...
    {
        var process = JsonFileOpener.OpenJsonFile(PathUtils.ConfigPath);
        Console.WriteLine("请编辑打开的Config文件并保存，保存后才能生效。");
        Console.WriteLine("设置已保存，按任意键返回主菜单。");
        Console.ReadKey();
        Configs.Init();
        Console.ReadKey();
    }

    private  static void PT字典转Mtool字典()
    {
